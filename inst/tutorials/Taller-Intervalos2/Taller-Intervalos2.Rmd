...
```{r ej1, exercise=TRUE, exercise.lines = 8}
...
```{r ej2, exercise=TRUE, exercise.lines = 8}
...
```{r eje2-solution}
cat("cálculo paramétrico")
x2=c(16.8, 17.2, 17.4, 16.9, 16.5, 17.1)
cat("IC :",t.test(x2,conf.level=0.99)$conf.int9)
cat("cálculo no paramétrico")
boot=sample(x2,7000,replace=TRUE)
b=matrix(boot,nrow=1000,ncol=7)
mx=apply(b,1,mean)
ic2=quantile(mx, probs=c(0.005,0.995))
...
x4=823
p4=x4/n4
cat("IC :", prop.test(x4,n4,0.95)$conf.int)
```
...
```{r ej5-solution}
cat("IC :",t.test(x5,coef.level=0.99)$conf.int)
```
...
```{r ej6-solution}
cat("IC :",t.test(x6,coef.level=0.95)$conf.int)
```
...
```{r ej7-solution}
cat("n = ",paqueteDEG::sizemu(1.96,40^2,15 ))
```
...
```{r ej8-solution}
cat("IC :",t.test(pro.estandar,pro.nuevo,var.equal=T)$conf.int)
```
...
```{r ej9-solution}
cat("a: ",prop.test(x=13,n=87,p=NULL,conf.level=0.95)$conf.int)
cat("b: ",paqueteDEG::sizep(1.96,0.50,0.03))
```
