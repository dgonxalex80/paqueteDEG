...
|:---------------------------|:--------------------------------------------------------------------|
| Datos                      |                                                                     |
| variable independiente     | `x=c(24.3,12.5,31.2,28,35.1,10.5,23.2,10,8.5,15.9,14.7,15)`         |
| variable dependiente       | `y=c(16.2,8.5,15,17,24.2,11.2,15,7.1,3.5,11.5,10.7,9.2)`            | 
| coef.correlación           | `cor(x,y)`                                                          |
| Ho: Rho=0                  | `cor.test(x,y)`                                                     |
| modelo                     | `regresion=lm(y ~ x)`                                               |
| resumen                    | `summary(regresion)`                                                |
| diagrama de dispersión     | `plot(x,y, xlab = "Ingresos", ylab = "Consumo",pch=21, bg="blue")`  |
| linea de regresión         | `abline(regresion)`                                                 |
| y estimada                 | `yhat=predict(regresion)`                                           |
| grafico de estimaciones    | `plot(x,yhat)`                                                      |
| IC  betas                  | `confint(regresion)`                                                |
| ANOVA                      | `anova(regresion)`                                                  |
| residuos                   | `residuos=rstandard(regresion)`                                     |
| valores ajustados          | `=fitted(regresion)`                                                |
| diag. yhat vs resid        | `plot(valores.ajustados, residuos)`                                 |
| prueba normalidad de u     | `qqnorm(residuos)`                                                  |
|                            | `qqline(residuos)`                                                  |
| instalacion paquete        | `install.packages("lmtest")`                                        |
| carga paquete              | `library(lmtest)`                                                   |
| Prueba de D-W              | `dwtest(regresion)`                                                 |
| Prueba de Goldfeld-Quandt  | `gqtest(y~x)`                                                       |
| Prueba RESET               | `resettest(y~x, power=2, type="regressor")`                         |  
| Prueba de White            | `modelo1=lm(residuos~x)`                                            |
|                            | `summary(modelo1)`                                                  |
| estimacion max.verosimulitud | `lm(y~x)$coefficients;emv`                                        | 
